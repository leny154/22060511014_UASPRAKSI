...
class Penilaian extends CI_Controller
{
    ...
    public function wp()
    {
        ...
        foreach ($query_kriteria->result() as $row) {
            $w[$row->id_kriteria] = round(($row->bobot / $bobot->total), 9);
            $rumus1 .= $row->bobot . ", ";
        ...
        foreach ($query_alt->result() as $row) {
            $jum_s = 1;
            $valid = true; // Tambahkan flag validasi
            ...
            foreach ($query_kriteria->result() as $row_k) {
                ...
                if ($res->num_rows() > 0) {
                    ...
                    if ($row_k->tipe == 'benefit') {
                        $temp_value = POW($res2['bobot'], $w[$row_k->id_kriteria]);
                    } elseif ($row_k->tipe == 'cost') {
                        $temp_value = POW($res2['bobot'], (-1 * $w[$row_k->id_kriteria]));
                    }

                    // Periksa validitas nilai
                    if (is_nan($temp_value) || is_infinite($temp_value) || $temp_value <= 0) {
                        $valid = false;
                        $temp_value = 0; // Anggap nol jika tidak valid
                    }

                    $jum_s *= $temp_value;
                    $var_rumus .= "(" . $res2['bobot'] . "<sup>" . ($row_k->tipe == 'cost' ? '-' : '') . $w[$row_k->id_kriteria] . "</sup>)";
                }
            }
            // Jika tidak valid, hasilkan S sebagai nol
            $s[$row->id_alternatif] = $valid ? round($jum_s, 9) : 0;
            $var_rumus .= " = " . $s[$row->id_alternatif] . "<br>";
            $jum_v += $s[$row->id_alternatif];
            $i++;
        }
        $i = 1;
        ...
        foreach ($query_alt->result() as $row) {
            ...
            $v[$row->id_alternatif]['nilai'] = $jum_v > 0 ? round(($s[$row->id_alternatif] / $jum_v), 9) : 0;
